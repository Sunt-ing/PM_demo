...
package com.dbg.patternmining.models.pruning;
...
import com.dbg.patternmining.models.CSP.VariablePair;
...
//		//edge label verify
//		HashMap<Integer, HashMap<Integer, ArrayList<Double>>> nodeAEdgeLabels= new HashMap<Integer, HashMap<Integer,ArrayList<Double>>>();//nodeID-->(edge labels)
//		HashMap<Integer, HashMap<Integer, ArrayList<Double>>> nodeBEdgeLabels= new HashMap<Integer, HashMap<Integer,ArrayList<Double>>>();//nodeID-->(edge labels)
//		for (int i = 0; i < cls.size(); i++) 
//		{
//			ConnectedComponent c = cls.get(i);
//			int nodeA=c.getIndexA();
//			int nodeB=c.getIndexB();		
//			EdgeType e = (EdgeType) qry.getListGraph().getEdgeLabel(nodeA, nodeB);//get edge label
...
//			}
//			ArrayList<Double> nodeAEdgelist = nodeAEdgemap.get(c.getLabelA());
...
//			
//			ArrayList<Double> nodeBEdgelist = nodeBEdgemap.get(c.getLabelB());
...
//				for (Iterator<Entry<Integer, ArrayList<Double>>> iterator = nodeAEdgeCons.entrySet().iterator(); iterator.hasNext();) 
//				{
//					Entry<Integer, ArrayList<Double>> entry =  iterator.next();
//					int label=entry.getKey();
//					Collection<Double> edgeLabel=entry.getValue();
//					if(!node.getAEdgeLabelsList(label).containsAll(edgeLabel))//verify if the edge labels of pattern are in data graph according to node
//					{isValidNode=false; break;}
//				}
//				
//				if(isValidNode && nodeBEdgeCons!=null)
//				for (Iterator<Entry<Integer, ArrayList<Double>>> iterator = nodeBEdgeCons.entrySet().iterator(); iterator.hasNext();) 
//				{
//					Entry<Integer, ArrayList<Double>> entry =  iterator.next();
//					int label=entry.getKey();
//					Collection<Double> edgeLabel=entry.getValue();
