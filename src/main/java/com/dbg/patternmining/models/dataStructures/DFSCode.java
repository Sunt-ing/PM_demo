...
import com.dbg.patternmining.models.decomposer.Decomposer;
import com.dbg.patternmining.models.search.SearchLatticeNode;
import com.dbg.patternmining.models.utilities.DfscodesCache;
...
//			System.out.println("noooooo!!");
	...
	@Override
	public Frequency upperBound(Integer kthFreq){
		nonCandidates = new HashMap<Integer, HashSet<Integer>>();
		Query q = new Query((HPListGraph<Integer, Double>) me);
		PatternHyperGraph UB = new PatternHyperGraph(singleGraph, q, kthFreq);

		IntFrequency upperBound = UB.getUpperBound();		
		if(upperBound.compareTo(new IntFrequency(kthFreq)) > 0) {
			DFSSearch UBConverter = new DFSSearch(UB, kthFreq, nonCandidates);
			this.UBConverter = UBConverter;
		}
	...
	@Override
	public Frequency oneEdgeUpperBound(Integer kthFreq){
		Query q = new Query((HPListGraph<Integer, Double>) me);
		oneEdgePatternHyperGraph UB = new oneEdgePatternHyperGraph(singleGraph, q, kthFreq);
	...
	@Override
	public final Frequency aftUBFrequency(Integer currentMinFreq) {
...
//			Query q = new Query((HPListGraph<Integer, Double>) me);
			...
			if (Settings.CACHING) {
				...
				for (int i = 0; i < maps.size(); i++) // iterate over edges
														// removed!!
				{
					HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> edgeRemoved = maps.get(i);

					for (Iterator<Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>> iterator = edgeRemoved
							.entrySet().iterator(); iterator.hasNext();) {
						Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> removedEdgeEntry = iterator.next();
						HPListGraph<NodeType, EdgeType> listGraph = removedEdgeEntry.getKey(); // each
																								// graph
																								// candidate
						String key = listGraph.toString();
						ArrayList<Integer> graphMappings = removedEdgeEntry.getValue();// pattern
																						// nodeID
																						// ~
																						// original
																						// ID
						...
						if (DfscodesCache.cache.containsKey(key)) {
							HashMap<Integer, HashSet<Integer>> nodeNonCandidates = DfscodesCache.cache.get(key); // node
																													// ~
																													// noncandidates
...
//			ConstraintGraph cg = new ConstraintGraph(singleGraph, q, nonCandidates);
//			DFSSearch df = new DFSSearch(cg, currentMinFreq, nonCandidates);

			UBConverter.searchExistances();
			currentVariables = UBConverter.getResultVariables();

			int freq = UBConverter.getFrequencyOfPattern();

			if (Settings.CACHING) {
				if (freq >= currentMinFreq)
					nonCandidates = UBConverter.getNonCandidates();
			}
			...
			System.out.println("MNI Frequency:" + freq);
	...
	@Override
	public final Frequency frequency(Integer currentMinFreq) {
	...
	private ArrayList<int[]> insList;
	public ArrayList<int[]> getDFSInsList(){
		return insList;
	}
