...
package com.dbg.patternmining.models.CSP;
...
import com.dbg.patternmining.models.automorphism.Automorphism;
...
public class DFSSearch
{
	...
	public DFSSearch(PatternHyperGraph php,int kthFreq, HashMap<Integer, HashSet<Integer>> nonCands) {
		if(!Settings.CACHING)
			nonCandidates=(HashMap<Integer, HashSet<Integer>>) nonCands.clone();
		else
			nonCandidates=nonCands;
		this.minFreqThreshold=kthFreq;
		variables = php.getVariables();
		qry = php.getQuery();
		result = new Variable[variables.length];
		for (int i = 0; i < variables.length; i++) {
			HashMap<Integer, myNode> list = new HashMap<Integer, myNode>();
			result[i] = new Variable(variables[i].getID(), variables[i].getLabel(), list,
					variables[i].getDistanceConstrainedWith(), variables[i].getDistanceConstrainedBy());
		}
		visitedVariables = new HashSet<Integer>();
		sOrder = new SearchOrder(variables.length);
	}
	
	public DFSSearch(oneEdgePatternHyperGraph php,int kthFreq, HashMap<Integer, HashSet<Integer>> nonCands) {
		if(!Settings.CACHING)
			nonCandidates=(HashMap<Integer, HashSet<Integer>>) nonCands.clone();
		else
			nonCandidates=nonCands;
	...
	public ArrayList<int[]> searchExistances()
	{
		ArrayList<int[]> insList = new ArrayList<int[]>();
